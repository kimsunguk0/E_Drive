#!/usr/bin/env python3
"""Durably supervise the four fixed MotionDrive V2 long-training jobs."""
...
DRIVER = "scripts/run_motiondrive_v2_long_training.py"
JOB_NAMES = ("long_s0", "long_s1", "holdout_s0", "holdout_s1")
ARM_SEEDS = {"long_s0": "0", "long_s1": "1",
...
def gpu_snapshot(gpu_uuids: set[str]) -> list[dict]:
    command = [
    ...
    for line in result.stdout.splitlines():
        ...
        if fields[1] in gpu_uuids:
            rows.append(dict(zip(
                ("index", "uuid", "name", "memory_total_mib", "memory_used_mib",
                 "memory_free_mib", "utilization_percent"), fields)))
    require({row["uuid"] for row in rows} == gpu_uuids,
            "Requested physical GPUs are missing")
    return rows
...
def validate_spec(spec: dict, cwd: Path, receipt: Path) -> tuple[list[dict], dict]:
    ...
    require(str((cwd / DRIVER).resolve()) in verified,
            "The executed long-training driver must be source-pinned")

    jobs = spec["jobs"]
    require(isinstance(jobs, list) and len(jobs) == 4,
            "Exactly four jobs required")
    require({job.get("name") for job in jobs} == set(JOB_NAMES),
            "Long-training job-name set mismatch")
    ...
    for raw_job in jobs:
        ...
        require(isinstance(gpu_uuid, str) and gpu_uuid.startswith("GPU-")
                and "," not in gpu_uuid and "\0" not in gpu_uuid,
                f"Invalid physical GPU UUID: {name}")
        require(isinstance(argv, list) and len(argv) >= 2
                ...
                and all(isinstance(value, str) and value and "\0" not in value
                        for value in argv),
                f"Invalid argv: {name}")
        require("--preflight-only" not in argv and "--smoke-only" not in argv,
                "Training argv contains diagnostic-only mode")
        controlled = exact_options(argv)
        require(controlled["--arm"] == name, f"Arm mismatch: {name}")
        require(controlled["--seed"] == ARM_SEEDS[name], f"Seed mismatch: {name}")
        ...
        require(argv_run.resolve() == run_dir.resolve(),
                f"Run-dir argv mismatch: {name}")
        ...
        for raw_result in job["result_paths"]:
            ...
            if not result.is_absolute():
                result = cwd / result
            require(result.resolve() not in seen_paths,
...
def run(spec_path: Path, receipt: Path) -> int:
    ...
    require(not receipt.exists() and not receipt.is_symlink(),
    ...
    gpu_uuids = {job["gpu_uuid"] for job in jobs}
    snapshots = gpu_snapshot(gpu_uuids)
    ...
    for gpu_uuid in sorted(gpu_uuids):
        ...
        required_free = 12000 * job_count + 8192
        require(int(by_uuid[gpu_uuid]["memory_free_mib"]) >= required_free,
                f"Insufficient aggregate guarded free memory: {gpu_uuid}")
        aggregate_memory_gate[gpu_uuid] = {
            "jobs": job_count, "allocator_cap_mib_each": 12000,
            "reserve_mib": 8192, "required_free_mib": required_free,
        }

    payload = {
        "schema_version": 1, "experiment": spec["experiment"],
        "status": "launching", "supervisor_pid": os.getpid(),
        "supervisor_pgid": os.getpgrp(), "cwd": str(cwd),
        "spec": str(spec_path), "spec_sha256": sha256(spec_path),
        ...
        "aggregate_memory_gate": aggregate_memory_gate,
    ...
    try:
        for job in jobs:
            try:
                ...
                payload["jobs"][job["name"]] = {
                    **job,
                    "environment": {**FIXED_ENV,
                                    "CUDA_VISIBLE_DEVICES": job["gpu_uuid"]},
                    "pid": process.pid, "pgid": process.pid,
                    "started_utc": timestamp(), "status": "running",
                    "return_code": None,
                }
                ...
                payload.update(ended_utc=timestamp(),
                               gpu_snapshot_after=gpu_snapshot(gpu_uuids))
                write_receipt(receipt, payload)
                return 1
    ...
    payload["status"] = "completed" if all(
        ...
        for job in payload["jobs"].values()) else "failed"
    payload.update(ended_utc=timestamp(),
                   gpu_snapshot_after=gpu_snapshot(gpu_uuids))
    write_receipt(receipt, payload)
