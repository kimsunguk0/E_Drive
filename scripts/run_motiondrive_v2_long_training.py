...
NAME = "motiondrive_v2_long_training"
ARMS = ("long_s0", "long_s1", "holdout_s0", "holdout_s1")
ARM_SEEDS = {"long_s0": 0, "long_s1": 1, "holdout_s0": 0, "holdout_s1": 1}
HOLDOUT_ARMS = frozenset(("holdout_s0", "holdout_s1"))
UPDATES = 68500
...
SOURCE_FILES = set(a2.SOURCE_FILES) | {
    ...
    "tests/test_motiondrive_v2_long_training.py",
}
...
def make_model(config):
    ...
    return install_shared_status_query(MotionDriveV2(config))
...
def prepare_model(payload, seed):
    ...
    require(not incompatible.missing_keys and not incompatible.unexpected_keys,
            "long-training A2 parent strict load failed")
    require(all(torch.equal(model.state_dict()[name], value)
                for name, value in payload["model"].items()),
            "long-training A2 parent tensors did not load exactly")
    require(any(name.startswith("shared_status_query_fusion.")
                for name in model.state_dict()),
            "long-training model lacks A2 query conditioning")
    return model, {
...
def validate_holdout_split(path, expected_sha256, split_manifest):
    ...
    require(len(full_train) == 203 and held_out | training == full_train,
            "long-training holdout artifact must partition train203")
...
def status_dataset(base, split, overlay):
    ...
    return a1.SharedStatusDataset(base, split, aligned, "provided_causal_5d")
...
def patched_runtime(arm, seed, overlay, holdout, expected_parent_sha,
    ...
    def train_inputs(batch, time_input="raw",
        ...
        return a1.shared_status_model_inputs(
            originals["train_inputs"], batch, time_input=time_input,
