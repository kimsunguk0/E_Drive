...
set -euo pipefail
SRC=/home/pm97/workspace/sukim/adcl/env_etri_extracted
...
grep -rl '^#!/opt/venv/bin' "$DEST/venv/bin" 2>/dev/null \
  | xargs -r sed -i "1s#^#!/opt/venv/bin/python.*##;1i#!$DEST/venv/bin/python" 2>/dev/null || true
...
if [ "$SKIP" != "--skip-verify" ]; then
  ...
  "$DEST/venv/bin/python" \
    /home/pm97/workspace/sukim/adcl/scripts/verify_etri_env.py
fi
