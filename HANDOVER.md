...
**2026-09-23 최종 공식 결과 EXT-FULL-v7 = 0.117066869 (사용자 보고, 제출 5/5 사용 완료):** K=15(종5×횡3) 영상 후보 + goal 5 s 종점 argmin, 1회 forward, `__flops__` 743,777,233,600. L2_1s/2s/3s = 0.06544/0.11530/0.17047. 이전 0.119691 대비 −2.19%. 제출 전 사용자 보고 4등 0.1174보다 낮다. 서버/쌍둥이 비 0.899(쌍둥이 V0 0.1302). 오늘 공식 점수는 0.133685 → 0.119691 → 0.117067이다. 원문 `reports/a2_final_push_20260923/SERVER_RESULT_EXT-FULL-v7.json`. 남은 제출 없음.

**2026-09-23 16:55 KST 마지막 제출 준비 완료 (제출 1회 남음, 업로드는 사용자):** 권장 EXT-FULL-v7(K=15, 5×3). 쌍둥이 V0 0.1302이고, 정지 29% 혼합에서도 0.1128로 s1(서버 0.119691)의 0.1133보다 좋다. 대안 EXT-FULL-v10(K=35)은 V0 0.1290이지만 정지 구간 0.0647로 s1 0.0522보다 나빠 테스트(goal<5 m 29%)에서 이점이 사라질 수 있다. 분해형·이웃 일관성·seed soup·선택 규칙 변형은 모두 v7을 넘지 못했다. 상세는 `reports/a2_final_push_20260923/RESULTS_KO.md` 5절. codex 리뷰 결론은 3등 가능성 낮음, 4등 불확실, 정지 비율 위험 확인. nuScenes는 H200에 전체가 풀려 있다(B200 다운로드 57 MB/s 가능, 오늘은 시간 부족).

**2026-09-23 11:45 KST 공식 결과 EXT-FULL-s1 = 0.119690721 (사용자 보고, 제출 4/5 사용, 남은 1회):** L-FULL6에 5초 후보 6개 + goal 종점 argmin 선택(ExtModel, 몸통 동결 2단계)을 붙인 1회 forward. 이전 0.133685 대비 −10.47%. held-out 쌍둥이 EXT-TWIN-s1 0.132550(부모 0.143554 대비 −0.011004, 11/11), 서버/쌍둥이 비 0.903. 규정 근거: 2026-09-23 오픈채팅 답변 — goal point만으로 좌표 변형 없이 후보 선택 허용, 별도 선택 네트워크·MLP 선택기·status/과거 기반 선택 금지, 영상→궤적 gradient가 이어지는 한 네트워크 필수(2-stage 학습 허용). 사용자 보고 순위권: UNICON 0.1097 / SAFE 0.1106 / 4등 0.1174. 결과 원문 `reports/a2_final_push_20260923/SERVER_RESULT_EXT-FULL-s1.json`. 진행 중: v4(3×3), v5(4 epoch), v6(LENW 부모), v7(5×3) 쌍둥이 — 마지막 1회는 쌍둥이 최선 레시피의 FULL로 결정.

**2026-09-23 10:25 KST final push, 슬롯 1 빌드 완료 (업로드는 사용자):** L-FULL6-s1 step38070 + Flip TTA, `reports/a2_final_push_20260923/sub_L-FULL6-TTA/package/submission.zip` sha256 `eedd7560…9791313`, 1,125 clip + `__flops__`1,460,089,722,240(2 forward). held-out 판정(`reports/a2_final_push_20260923/RESULTS_KO.md`): TTA는 L-FULL6의 정확한 held-out 쌍둥이 L-TRAIN3106에서 0.143554→0.139214(−0.004340, 9/11), LENW+TTA 0.138376(TTA 대비 추가 −0.000839, 9/11). SWA·checkpoint 앙상블·계보 간 앙상블은 이득 없음. 같은 부모·같은 순서의 동급 형제 soup(F-CTRL+F-FLOW)만 −0.000754(8/11). 횡방향 y×1.03 사후 보정은 LOSO −0.0005 수준. 학습 중: GPU6 L-FULL6-LENW-s1, GPU4 L-FULL6-LENW-s2(seed2), GPU3 L-TRAIN3106-LENW-s2(soup 쌍둥이 판정용), 모두 ~15:30~16:00 종료. 슬롯 2는 그 결과로 결정. 공식 제출 남은 횟수 2(슬롯 1 업로드 전 기준).

**2026-09-22 16:52 KST sampler/Adam 세 arm 완료:** GPU1 A-REPLAY-FRESH0.143904217 / GPU2 B-NEW-FRESH0.143019460 / GPU3 C-NEW-ADAM0.143162506. 모두5,232update·train83,700행정확히1회·마지막batch4·NaN/Inf0, B/C전step row/증강표본hash일치·Adam377개최종step복원확인. 새순서/증강B는반복A대비-0.000884757(CI[-0.001779023,-0.000046242])이나 부모C-DECSPLIT0.142952519는넘지못했다. Adam유지C는B보다+0.000143046(CI0포함);100step초기악화는작았지만최종이득없음. 예정모든평가가부모보다나빴고trainprobe만최종개선. 일반주행공통진행량MAE는B에서0.088821→0.088413로0.46%감소에그쳤다. 이번설정종료·새승격/FULL/연장/제출없음·기존DEV부모와별도H편입0.138823보존. [최종판정](reports/a2_sampler_adam_20260922/TERMINAL_REVIEW_KO.md), [terminal 수치](reports/a2_sampler_adam_20260922/result_step5232.json), [실행검증](reports/a2_sampler_adam_20260922/terminal_execution_checks.json), [가중치색인](reports/a2_sampler_adam_20260922/candidate_registry.json).


**2026-09-22 16:18 KST GPU0 H 편입 PROGRESS 완료:** train310+H29=339scene/91,530행 추가34,326update(6회 노출)를 마쳤고 terminal V0 **0.138823077**이 전체 예정 평가 중 최저다. 시작0.151178860 대비0.012355782(8.17%) 개선, 이전4회 최저0.139249874 대비0.000426797 개선. NaN/Inf0·checkpoint SHA·저장예측1,998행 CPU 재계산 확인. 고정 train310/H probe도 개선됐고 이번 일정 내 지속적 과적합 시점은 미관측이다. H 추가와 긴 continuation이 함께 바뀌었으므로 같은 train310 DEV 구조대조나 서버점수 환산으로 해석하지 않는다. 모든가중치/terminal/best 보존·추가학습/FULL/업로드 자동착수없음. GPU1/2/3 sampler/Adam대조는별도진행중. [최종판정](reports/a2_nearfull_overfit_20260922/TERMINAL_REVIEW_KO.md), [재계산·세션CI](reports/a2_nearfull_overfit_20260922/terminal_recheck.json), [학습곡선](reports/a2_nearfull_overfit_20260922/RESULTS_KO.md), [후보](reports/a2_nearfull_overfit_20260922/best_candidate.json).


**2026-09-22 15:06 KST 샘플·증강 / Adam 대조 본 학습 시작:** GPU1 A-REPLAY-FRESH(기존 순서·증강+fresh Adam), GPU2 B-NEW-FRESH(새 순서·epoch4 증강+fresh Adam), GPU3 C-NEW-ADAM(B와 동일 입력+부모 Adam moment/step 유지). 같은 C-DECSPLIT DEV0.142952519에서 train310/83,700행 전체 한 바퀴·5,232update, batch16/micro8/마지막4행·기존 loss·LR1e-6/1e-5·warmup100·cosine·clip5. 세 arm 모두5update·원본A 입력재현·B/C 입력일치·Adam377개 복원·strict reload 차이0을 통과했다. 본 학습 초기V0는 모두0.142952521이고 실제 update/유한loss·gradient를 확인했다. B-A는 순서+증강 정책 묶음, C-B는 새stream에서 optimizer 상태 비교이며 terminal이 주 비교다. Explicit permutation/소비 cursor를 checkpoint에 보존한다. 평가0/100/500/2,616/5,232. GPU0 near-FULL은 변경하지 않았고 자동 FULL/연장/제출은 없다. [실행 조건](reports/a2_sampler_adam_20260922/EXECUTION_KO.md), [검증](reports/a2_sampler_adam_20260922/tests_summary.json), [실제 launch·ETA](reports/a2_sampler_adam_20260922/launch_health.json), [결과·현황](reports/a2_sampler_adam_20260922/RESULTS_KO.md).


**2026-09-22 14:19 KST 진행량 전용 최적화 DEV 완료:** O-CTRL0.143970230 / O-LENGTH0.143668267로 부모 C-DECSPLIT0.142952519를 넘지 못했다. 두 arm 3,426update·NaN/Inf0·전step row stream 일치, LENGTH의 고정 파라미터/buffer와 scene/motion/state/history/heading 출력 불변 확인. LENGTH는CTRL보다0.000301963 낮지만 CI0포함이며 부모 대비길이MAE·종방향·일반주행오차가 모두 소폭악화했다. Trainprobe만소폭개선. 이번레시피종료·부모보존·새FULL/연장/제출없음. GPU3진단이반복적충돌을뒷받침하지않아MOTION0미실행. 별도GPU0 H편입은진행중이며20,024step에서V00.141245081;추가H데이터가포함된별도비교다. [최종 판정](reports/a2_progress_optimization_20260922/TERMINAL_REVIEW_KO.md), [수치·세션/그룹](reports/a2_progress_optimization_20260922/result_step3426.json), [보존 가중치](reports/a2_progress_optimization_20260922/candidate_registry.json).

**2026-09-22 12:04 KST C-DECSPLIT 길이 전용 최적화 착수:** 사용자 지정 GPU1 O-CTRL / GPU2 O-LENGTH를 같은 DEV 부모0.142952519에서 3,426update 추가 학습한다. LENGTH는 613,377개 길이 query/decoder/read/head만 학습하고 공유 표현·방향·buffer를 고정한다. 5-step·고정 출력/해시·strict reload 검사를 통과했다. GPU3 train8배치 gradient 진단은 optimizer update0으로 완료: backbone P/M cosine중앙값+0.0898(음수1/8), motion공유부+0.0206(2/8), global P norm24.2946/M6.3376. 반복적 보조-task 충돌 근거가 약해 조건부 O-MOTION0는 미실행. 평가0/1,142/2,284/3,426, terminal主비교·동일row/증강/고정trainprobe 확인. 초기 입력감사 field alias 오류는 optimizer update 전에 수정·실패run보존·재검사했다. GPU0 near-FULL은 계속 별도 진행하며 자동FULL/제출/연장은 없다. [실행·검증](reports/a2_progress_optimization_20260922/EXECUTION_NOTES_KO.md), [최신 상태](reports/a2_progress_optimization_20260922/run_index.json), [결과](reports/a2_progress_optimization_20260922/RESULTS_KO.md), [진단](reports/a2_progress_optimization_20260922/gradient_probe.json).

**2026-09-22 11:28 KST C-DECSPLIT 잔여 오차 CPU 분석 완료:** 저장 V0 1,998행/37scene/11session을 같은 CTRL·M-NATIVE·H4-PROGRESS와 대조했다. PREFIX0.142952519, 첫2초 기여71.6%, 일반주행95.8%, 속도 변화가 작은 직선 근방1,179행만52.1%다. 종/횡 MAE0.118474/0.060429m; 횡 절대오차78.0%가 GT 방향5도 미만 구간에서 발생한다. 길이GT 치환0.057333·방향GT 치환0.115078은 배포 불가능한 출력 진단이며 성능 예측이 아니다. CTRL 대비이득0.000269615의83.6%가session092에 집중;CI0포함. 가감속 집단에서 미래 속도 변화를 완만하게 예측하는 경향은 있으나 타이밍 원인 확정 불가. 새 학습/GPU추론/FULL/업로드 없이 분석했고 GPU0 near-FULL 작업은 변경하지 않았다. [상세 보고서](reports/a2_decsplit_error_20260922/RESULTS_KO.md), [전체 통계](reports/a2_decsplit_error_20260922/STATISTICS.json), [도표](reports/a2_decsplit_error_20260922/error_breakdown.png), [행별 오차](reports/a2_decsplit_error_20260922/ROWS.csv).


**2026-09-22 10:55 KST H 편입 PROGRESS 관찰 시작:** 사용자 요청으로 GPU0에서 P-NEARFULL-H-s1을 시작했다. 원래 PROGRESS DEV step20,554/0.151178860의 같은 graph를 train310+H29=339scene/91,530행에 추가 34,326update(6회 노출), backbone1e-6/head1e-5·fresh AdamW·warmup100·고정 cosine으로 학습한다. V0 37scene/1,998행/11session의 모든 scene을 학습에서 제외했고 초기 점수 재현·실제 H 포함 5update·strict reload를 통과했다. 약0.5회 노출마다 V0와 고정 train310/H 표본을 평가하고 모든 checkpoint·V0 best를 보존한다. 최종까지 개선되면 과적합 시점 미관측으로 기록한다. H는 이 새 계보에서 학습 데이터이며 FULL weights/features는 반입하지 않았다. 새 FULL/제출 자동 실행 없음. [실행 계약](reports/a2_nearfull_overfit_20260922/EXECUTION_KO.md), [진행·ETA](reports/a2_nearfull_overfit_20260922/launch_health.json), [학습 곡선](reports/a2_nearfull_overfit_20260922/RESULTS_KO.md), [현재 판정](reports/a2_nearfull_overfit_20260922/overfit_status.json).
