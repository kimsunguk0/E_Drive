# 오프라인 캐시(768x432) 로더 변형. Phase G 비용 측정용.
#
# **이름 주의**: 이건 C0-T가 아니다. 모델에 ego goal 입력 경로가 **없다**
# (VAD_head.py/VAD.py 에 goal 관련 코드 0건, CustomCollect3D 키에도 없음).
# 조건화는 3-way command(`ego_fut_cmd`) 하나뿐이므로 정확한 성격은
# 'ETRI temporal VAD bootstrap' 이다. 실제 C0-T는 goal encoder + goal-conditioned
# visual attention 을 붙인 순간부터 시작이다.
# Undistort/Crop/RandomScale 을 제거하고 CachedImageGeometry 로 행렬만 맞춘다.
# C0-T: use_can_bus=False 로 can_bus learned embedding 차단 (VAD_transformer.py 패치와 짝).
# 원본 config 와의 유일한 차이는 이 세 줄의 교체와 data_root 다.
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        transformer=dict(
            ...
            map_num_vec=map_num_vec,
            map_num_pts_per_vec=map_fixed_ptsnum_per_pred_line,
            # 기하 정렬은 유지: 아래 두 플래그는 can_bus[0],[1],[-2],[-1]만 직접 읽는다.
            rotate_prev_bev=True,
            use_shift=True,
            # 컴플라이언스(8/18 공지 3조): can_bus 18차원 learned embedding 차단.
            # accel/rotation_rate/velocity가 MLP를 타고 BEV 쿼리에 가산되는 경로였다.
            # VAD_transformer.py의 `if self.use_can_bus:` 패치와 짝을 이룬다.
            use_can_bus=False,
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file='/tmp/pm97/data/etri/pkl/vad_etri_infos_temporal_train.pkl',
    ...
    val=dict(type=dataset_type,
             data_root=data_root,
             pc_range=point_cloud_range,
             ann_file=data_root + 'vad_etri_infos_temporal_val.pkl',
