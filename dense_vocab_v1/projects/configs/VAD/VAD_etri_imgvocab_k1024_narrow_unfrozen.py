# 공정 대조군: pc_range 는 기존 ±30 유지, **동결만 해제**.
# wide 판(±60)과 비교하면 남는 차이가 pc_range 하나뿐이므로 커버리지 인과를 가른다.
# (동결판 ±30 은 학습 파라미터가 0.73 M 뿐이라 wide 43.07 M 과 교란된다.)
...
log_level = 'INFO'
work_dir = '/tmp/pm97/work_dirs/imgvocab_k1024_narrow_unfrozen'
...
model = dict(
    type='VAD',
    freeze_except_goal_decoder=False,   # wide 판과 단일 변수(pc_range)만 다르게 하려면 동결도 풀어야 한다
    use_grid_mask=True,
    ...
    pts_bbox_head=dict(
        ...
        goal_decoder=dict(
            type='ImageOnlyVocabularyDecoder',
