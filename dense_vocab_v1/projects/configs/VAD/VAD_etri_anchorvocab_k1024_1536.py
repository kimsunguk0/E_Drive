# 해상도 1536x864 판 (기존 768x432). 캐시 crop 은 동일하고 scale 만 0.4 -> 0.8.
# 근거: 오차의 77~86%가 종방향이고 종방향은 먼 거리 판단인데, 35 m 앞 물체가
# 768x432 에서는 몇 픽셀뿐이다. pc_range 확장(격자만 넓힘)은 통제 실험에서
# 효과가 없었으나(±60 ~ ±30), 해상도는 정보량 자체를 늘린다.
# batch 4 -> 2 (VRAM). lr 은 건드리지 않는다 -- 임의 조정 금지 규칙.
...
train_pipeline = [
    ...
    dict(type='CachedImageGeometry', scale=0.8),
    dict(
...
data = dict(
    samples_per_gpu=2,   # 1536x864 는 픽셀 4배 -> batch 반으로
    workers_per_gpu=4,
    train=dict(
        type='VADCustomETRIDataset',
        data_root='/tmp/pm97/data/etri/',
        ann_file='/tmp/pm97/data/etri/pkl/etri_train330_goal_1536.pkl',
        pipeline=[
            dict(type='LoadMultiViewImageFromFiles', to_float32=True),
            dict(type='CachedImageGeometry', scale=0.8),
            dict(
    ...
    val=dict(
        type='VADCustomETRIDataset',
        ann_file='/tmp/pm97/data/etri/pkl/etri_val38_goal_1536.pkl',
        pipeline=[
            dict(type='LoadMultiViewImageFromFiles', to_float32=True),
            dict(type='CachedImageGeometry', scale=0.8),
            dict(
...
log_level = 'INFO'
work_dir = '/tmp/pm97/work_dirs/anchorvocab_k1024_wide_1536'
load_from = '/home/pm97/workspace/sukim/adcl/b200_eval/v2a_goal/epoch2/epoch_2.pth'
resume_from = None
workflow = [('train', 1)]
plugin = True
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        goal_decoder=dict(
            ...
            bev_w=200,
            use_global=True,
            anchors_path='/home/pm97/workspace/sukim/adcl/artifacts/dense_vocab/etri_train330_vocab_k1024.npy',
            n_head=8,
            loss_weight=1.0,
...
TRAIN330 = '/tmp/pm97/data/etri/pkl/etri_train330_goal_1536.pkl'
VAL38 = '/tmp/pm97/data/etri/pkl/etri_val38_goal_1536.pkl'
gpu_ids = range(0, 1)
