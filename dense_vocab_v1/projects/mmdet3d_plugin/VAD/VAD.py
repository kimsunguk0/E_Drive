...
class VAD(MVXTwoStageDetector):
    ...
    def __init__(self,
                 ...
                 train_only_goal_residual=False,
                 fut_ts=6,
                 fut_mode=6,
                 img_norm_cfg=None
                 ):
        ...
        self.expose_image_evidence = bool(expose_image_evidence)
        self.freeze_except_goal_decoder = bool(freeze_except_goal_decoder)
        self.train_only_goal_residual = bool(train_only_goal_residual)
        if self.freeze_except_goal_decoder:
            trainable = 0
            for name, parameter in self.named_parameters():
                keep = name.startswith('pts_bbox_head.goal_decoder.')
                if keep and self.train_only_goal_residual:
                    # Phase C: base scorer(champion) 도 동결, velocity/stop residual 만 학습
                    keep = ('goal_decoder.vel_' in name) or ('goal_decoder.stop_' in name)
                parameter.requires_grad = keep
                trainable += parameter.numel() if keep else 0
            if trainable == 0:
                raise RuntimeError(
                    'freeze_except_goal_decoder found no trainable parameters')
            tag = 'residual only' if self.train_only_goal_residual else 'goal decoder'
            print(f'[VAD] frozen visual trunk; {tag} trainable params={trainable:,}')
        ...
        if img_norm_cfg is not None:
            self.img_norm_to_rgb = img_norm_cfg.get('to_rgb', True)
            self.register_buffer(
                'img_norm_mean',
                torch.tensor(img_norm_cfg['mean'], dtype=torch.float32).view(1, 3, 1, 1),
                persistent=False)
            self.register_buffer(
                'img_norm_std',
                torch.tensor(img_norm_cfg['std'], dtype=torch.float32).view(1, 3, 1, 1),
                persistent=False)
    ...
    def extract_img_feat(self, img, img_metas, len_queue=None):
        ...
        if img is not None:
            ...
            if self.img_norm_cfg is not None:
                if self.img_norm_to_rgb:
                    img = img[:, [2, 1, 0], :, :]
                img = (img - self.img_norm_mean) / self.img_norm_std
            if self.use_grid_mask:
                img = self.grid_mask(img)

            # Retained only for the opt-in C0-T v2a V stream.  The local
            # content gate makes an all-zero camera tensor annihilate backbone
            # bias/BN/FPN offsets exactly, without a second backbone forward.
            image_content_source = img if self.expose_image_evidence else None
        ...
        img_feats_reshaped = []
        image_content_feats_reshaped = []
        for img_feat in img_feats:
            BN, C, H, W = img_feat.size()
            if self.expose_image_evidence:
                gate = F.interpolate(
                    image_content_source.abs().mean(dim=1, keepdim=True),
                    size=(H, W), mode='bilinear', align_corners=False)
                gate = gate / gate.mean(
                    dim=(-2, -1), keepdim=True).clamp_min(1e-6)
                centered_feat = img_feat - img_feat.mean(
                    dim=(-2, -1), keepdim=True)
                image_content_feat = centered_feat * gate
            if len_queue is not None:
                img_feats_reshaped.append(img_feat.view(int(B/len_queue), len_queue, int(BN / B), C, H, W))
                if self.expose_image_evidence:
                    image_content_feats_reshaped.append(image_content_feat.view(
                ...
                if self.expose_image_evidence:
                    image_content_feats_reshaped.append(image_content_feat.view(
                        B, int(BN / B), C, H, W))
        if self.expose_image_evidence:
            return img_feats_reshaped, image_content_feats_reshaped
    ...
    def forward_pts_train(self,
        ...
        outs = self.pts_bbox_head(pts_feats, img_metas, prev_bev,
                                  ...
                                  ego_fut_goal=ego_fut_goal, ego_fut_cmd=ego_fut_cmd)
        loss_inputs = [
    ...
    def obtain_history_bev(self, imgs_queue, img_metas_list):
        ...
        with torch.no_grad():
            ...
            extracted = self.extract_feat(img=imgs_queue, len_queue=len_queue)
            if self.expose_image_evidence:
                img_feats_list, image_content_feats_list = extracted
            else:
                img_feats_list = extracted
                image_content_feats_list = None
            for i in range(len_queue):
                img_metas = [each[i] for each in img_metas_list]
                # img_feats = self.extract_feat(img=img, img_metas=img_metas)
                img_feats = [each_scale[:, i] for each_scale in img_feats_list]
                image_content_feats = None if image_content_feats_list is None else [
                    each_scale[:, i] for each_scale in image_content_feats_list]
                prev_bev = self.pts_bbox_head(
                    img_feats, img_metas, prev_bev,
                    image_content_feats=image_content_feats, only_bev=True)
            self.train()
            return prev_bev
    ...
    def forward_train(self,
        ...
        extracted = self.extract_feat(img=img, img_metas=img_metas)
        if self.expose_image_evidence:
            img_feats, image_content_feats = extracted
        else:
            img_feats = extracted
            image_content_feats = None
        losses = dict()
        losses_pts = self.forward_pts_train(img_feats, gt_bboxes_3d, gt_labels_3d,
                                            ...
                                            ego_lcf_feat=ego_lcf_feat, ego_fut_goal=ego_fut_goal,
                                            image_content_feats=image_content_feats,
        ...
        **kwargs
        ...
        if self.prev_frame_info['prev_bev'] is not None:
            ...
            img_metas[0][0]['can_bus'][-1] = \
                (img_metas[0][0]['can_bus'][-1] - self.prev_frame_info['prev_angle'] + 180) % 360 - 180
        else:
        ...
        new_prev_bev, bbox_results = self.simple_test(
            ...
            ego_fut_goal=None if ego_fut_goal is None else ego_fut_goal[0],
            gt_attr_labels=gt_attr_labels,
            **kwargs
        )
    ...
    def predict_candidates(self, img, img_metas, prev_bev=None,
        ...
        ego_his/ego_lcf/gt 가 **없다**(kwargs 도 없다 — 우회 전달 차단).
        history_img/calibration/history_alignment 은 미래 sparse trunk 의 고정-기하
        정렬 placeholder(현재 미사용).

        image -> extract_feat -> head(goal/cmd=None) 로 vocab logits 만 얻고,
        decoder.build_candidates 로 N=12 완성 후보를 만든다. detection gt/metric 경로를
        타지 않으므로 planner/scorer graph 에 gt/goal/cmd 가 들어가지 않는다.

        img: [B,N,C,H,W], img_metas: list[dict] (head 형식, forward_test 가 언래핑한 것).
        ...
        extracted = self.extract_feat(img=img, img_metas=img_metas)
        if getattr(self, 'expose_image_evidence', False):
            x, image_content_feats = extracted
        else:
            x, image_content_feats = extracted, None
        holder = []
        h = gd.register_forward_hook(lambda m, i, o: holder.append(o[1]))
        try:
            # goal/cmd/status/his/lcf 를 전달하지 않는다(None). decoder 는 image evidence 만 읽는다.
            self.pts_bbox_head(x, img_metas, prev_bev=prev_bev,
                               image_content_feats=image_content_feats,
                               ego_his_trajs=None, ego_lcf_feat=None,
                               ego_fut_goal=None, ego_fut_cmd=None)
        finally:
            h.remove()
        return gd.build_candidates(holder[-1])       # [B,K] -> dict(N=12)
