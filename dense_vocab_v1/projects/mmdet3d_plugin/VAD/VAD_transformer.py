...
class VADPerceptionTransformer(BaseModule):
    ...
    def __init__(self,
                 ...
                 detach_content_addressing=False,
        ...
        # content 분기가 trunk의 sampling 주소(`sampling_offsets`/`attention_weights`)에
        # gradient를 주지 않게 한다. 값의 순수성과는 무관하고 학습 안정성 문제다.
        self.detach_content_addressing = bool(detach_content_addressing)
        self.two_stage_num_proposals = two_stage_num_proposals
        self.rotate_center = rotate_center
        self.map_num_vec = map_num_vec
        self.map_num_pts_per_vec = map_num_pts_per_vec
        self.init_layers()
        self.set_expose_image_evidence(self.expose_image_evidence)

    def set_expose_image_evidence(self, enabled):
        """Toggle the opt-in SCA evidence branch across encoder layers."""
        self.expose_image_evidence = bool(enabled)
        for module in self.encoder.modules():
            if isinstance(module, SpatialCrossAttention):
                module.detach_content_addressing = getattr(
                    self, 'detach_content_addressing', False)
                module.set_expose_image_evidence(self.expose_image_evidence)

    def get_image_evidence(self):
        """Return current frame evidence from the last spatial encoder layer."""
        if not self.expose_image_evidence:
            return None
        for layer in reversed(self.encoder.layers):
            for attention in reversed(layer.attentions):
                if isinstance(attention, SpatialCrossAttention):
                    if attention.last_image_evidence is None:
                        raise RuntimeError('spatial image evidence was not populated')
                    return attention.last_image_evidence
        raise RuntimeError('no SpatialCrossAttention exists in the encoder')
    ...
    def get_bev_features(
        ...
        # add can bus signals
        #
        # 컴플라이언스 패치 (2026-08-21) -- 8/18 공지 3조.
        # can_bus 18차원에는 [7:10] accel, [10:13] rotation_rate, [13:16] velocity가
        # 들어 있고(etri_vad_converter.py:264-270), 원본 코드는 이 18차원 전체를
        # MLP로 256차원에 올려 BEV 쿼리에 **가산**했다. 영상과 독립적인 additive
        # path이므로 "최종 궤적은 영상 인식 결과로부터 도출" 요건을 위협한다.
        # 실측: velocity만으로 가중 L2 0.3715, goal+velocity 0.1297 -- 신호가 실재한다.
        #
        # `* self.use_can_bus`로 0을 곱하는 원본 방식은 MLP를 **여전히 호출**하므로
        # forward-hook 감사가 0을 못 만든다. 아예 분기로 감싼다.
        # use_shift / rotate_prev_bev는 별도 플래그이므로 기하 정렬은 그대로 살아 있다
        # (위쪽 shift·rotate 블록은 can_bus[0],[1],[-2],[-1]만 직접 읽는다).
        if self.use_can_bus:
        ...
        for lvl, feat in enumerate(mlvl_feats):
            ...
            if self.expose_image_evidence:
                if image_content_feats is None:
                    raise ValueError('image_content_feats is required for C0-T v2a')
                content_feat = image_content_feats[lvl]
                if content_feat.shape != mlvl_feats[lvl].shape:
                    raise ValueError(
                        'content/trunk feature mismatch at level {}: {} vs {}'.format(
                            lvl, tuple(content_feat.shape), tuple(mlvl_feats[lvl].shape)))
                # No camera, level, or positional embedding is ever added to
                # this tensor.  It is the separate V stream only.
                content_feat = content_feat.flatten(3).permute(1, 0, 3, 2)
                image_content_flatten.append(content_feat)

        feat_flatten = torch.cat(feat_flatten, 2)
        if self.expose_image_evidence:
            image_content_flatten = torch.cat(image_content_flatten, 2)
        ...
        if self.expose_image_evidence:
            image_content_flatten = image_content_flatten.permute(
                0, 2, 1, 3)
        ...
        encoder_kwargs = dict(
            ...
            **kwargs)
        if self.expose_image_evidence:
            encoder_kwargs['image_content_value'] = image_content_flatten
