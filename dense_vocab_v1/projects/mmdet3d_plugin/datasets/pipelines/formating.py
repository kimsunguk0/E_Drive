...
class CustomDefaultFormatBundle3D(DefaultFormatBundle3D):
    ...
    def __call__(self, results):
        ...
        if self.with_ego:
            ...
            # ego 5초 goal (C0-T). 주최측이 test clip의 ego_pose.parquet frame 50 으로
            # 명시적으로 주는 입력이다. `ego_fut_cmd`와 같은 [1,1,D] 규약을 따른다.
            if 'ego_fut_goal' in results:
                results['ego_fut_goal'] = DC(
                    to_tensor(results['ego_fut_goal'][None, None, ...]), stack=True)
