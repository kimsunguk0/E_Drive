...
class LiDARInstanceLines(object):
    ...
    def __init__(self, 
        ...
        self.code_dim = len(instance_line_list[0].coords[0]) \
            if len(instance_line_list) != 0 else 2
    ...
    def start_end_points(self):
        ...
        """
        # in-patch lane 0개 프레임: 빈 텐서를 낸다 (assert 금지).
    ...
    def fixed_num_sampled_points(self):
        ...
        for instance in self.instance_list:
            ...
            sampled_points = np.array([list(instance.interpolate(distance).coords) for distance in distances]).reshape(-1, self.code_dim)
    ...
    def shift_fixed_num_sampled_points(self):
        ...
        """
        # in-patch lane 0개 프레임. num_shifts는 아래 로직이 line일 때
        # fixed_num - 1 개를 만들므로 그 규약을 따른다.
        if len(self.instance_list) == 0:
            return torch.zeros((0, max(self.fixed_num - 1, 1), self.fixed_num, 2),
                               dtype=torch.float32)
    ...
    def shift_fixed_num_sampled_points_v2(self):
        ...
        # in-patch lane 0개 프레임: 빈 텐서를 낸다 (assert 금지).
        if len(self.instance_list) == 0:
            return torch.zeros((0, max(self.fixed_num - 1, 1), self.fixed_num, 2), dtype=torch.float32)
        instances_list = []
        for instance in self.instance_list:
            distances = np.linspace(0, instance.length, self.fixed_num)
            ...
            if multi_shifts_pts_tensor.shape[0] < final_shift_num:
                padding = torch.full([final_shift_num-multi_shifts_pts_tensor.shape[0],self.fixed_num,coords_num], self.padding_value)
...
class VADCustomNuScenesDataset(NuScenesDataset):
    ...
    def union2one(self, queue):
        ...
        for i, each in enumerate(queue):
            ...
            if i == 0:
                ...
                metas_map[i]['can_bus'][-1] = \
                    (metas_map[i]['can_bus'][-1] - prev_angle + 180) % 360 - 180
    ...
    def get_ann_info(self, index):
        ...
        # gt_names가 **완전히 빈** 프레임(330 시나리오 전체에 존재)에서
        # np.array([]) 는 float64가 되고, 이후 int64 labels 텐서에 대입하다
        # 'Index put requires the source and destination dtypes match' 로 죽는다.
        # 8개 시나리오에는 이 케이스가 없어 여태 드러나지 않았다.
