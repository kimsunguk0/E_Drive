...
현재 자원 허용 범위(20:57 KST 갱신): 사용자 후속 지시에 따라 GPU0–5.
다음 평가에는 실제 유휴를 확인한 GPU4·5를 우선 사용한다. GPU6·7 및 타 작업
중지는 허용하지 않는다. 아래 시간별 기록의 GPU0–3 제한은 당시 권한을 뜻한다.

최신 결과(22:31 KST): GPU4·5의 P3 상태-query r2 두 팔은 actualrc0로1000step 완주했다.
LAST control0.445977 / state0.437941, 차이−0.008037(11세션95%CI[−0.013904,−0.002724]).
소폭 이득은 관측됐으나 사전−0.01 진전 gate 미달이므로 대규모 확대/채택은 하지 않는다.
초기 두 r1은 동결/비동결 수치 조건 차이 때문에 step0에서 실패했고, 동결 전체1998프레임
legacy/control/state의1500 full-forward byte-parity를 확인한 별도 r2로 재실행했다.
실제 결과·미완료 항목은 `MOTIONDRIVE_V2_P3_RESULTS_20260907.md`에 정리했다.
GPU4·5는 종료 후 유휴이고,0–3 기존 작업 및6·7 미사용을 유지한다.
...
## 20:57 KST: P2 종료 상태 및 유휴 GPU4·5 추가 허용

P2 학습 프로세스는 모두 종료했다. C0/T0·C0/T1·C1/T1은 actual rc0이며,
C1/T0는 LAST3000/마지막 평가 저장 후 native SIGSEGV로 actual rc-11,
supervisor139로 자연 종료했다. 수동 신호와 OOM pressure event는 없었다.
원4판 clean-exit gate는 실패로 유지한다. 저장 완료를 정상 OS 종료로 대신하지 않는다.

사용자가 "비어있는 idle인 gpu 써도 돼 4,5번 써라"라고 권한을 넓혔다.
실제 GPU4·5 모두 메모리0MiB/util0%/compute PID 없음을 확인했다.
원0–3의 다른 작업을 중지하지 않고, 정상3팔의 분리 진단을 GPU4·5에 배치한다.
물리 GPU 선택만 wrapper에서 분리하며 학습계보/수치조건/메모리한도/실제종료 검사는
보존한다. 정확한 계획은 `MOTIONDRIVE_V2_P2_EXIT_INCIDENT_20260907.md`를 따른다.
이 기록 시점에 후속 GPU 평가는 아직 실행하지 않았다.
