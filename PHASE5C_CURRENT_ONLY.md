...
# ⑤-C4 — teacher history-depth curve: current-only 가설 확정 실패 (2026-09-05)

## 결과 (val38 frame>=30, n=2052, 동일 프레임·체크포인트·selector·가중)

| depth | top1 | o@3 | o@12 | slO@12 | real λ0 | real λ0.1 | stop | accel | left | right |
|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 0 (순수 t0) | 7.8962 | 7.7056 | 7.2830 | **7.2802** | 7.6384 | 7.6390 | 3.805 | 6.829 | 3.706 | 12.798 |
| 1 | 4.2508 | 2.7784 | 1.1310 | **1.1142** | 1.5502 | 1.5577 | 1.488 | 1.992 | 1.013 | 1.804 |
| 2 | 0.9569 | 0.6590 | 0.4659 | **0.4485** | 0.7617 | 0.7642 | 1.195 | 0.605 | 0.732 | 0.436 |
| 3 | 0.4341 | 0.3196 | 0.2002 | **0.1785** | 0.2833 | 0.2777 | 0.077 | 0.418 | 0.669 | 0.325 |
| 6 (champion) | 0.3516 | 0.2500 | 0.1615 | **0.1446** | 0.2476 | 0.2392 | 0.080 | 0.398 | 0.657 | 0.330 |

depth N = 매 target frame 에서 reset 후 과거 N장(interval 5)을 먹인 뒤 target 을 forward.
depth 0 = prev_bev 없이 그 한 장만.

## 판정: current-only 가설 실패

depth 0 의 slO@12 가 7.28 로, 사전 고정한 판정선 0.40 을 훨씬 넘는다. 그리고
depth 2~6 에서 극적으로 개선된다. **t0 한 장에는 필요한 정보가 없다.**

개선 폭의 분포:

| 구간 | slO@12 변화 | 전체 대비 |
|---|---:|---:|
| depth 0 → 1 | 7.28 → 1.11 | 85% |
| depth 1 → 2 | 1.11 → 0.449 | 9% |
| depth 2 → 3 | 0.449 → 0.179 | 3.7% |
| depth 3 → 6 | 0.179 → 0.145 | 0.5% |

**depth 3 에서 사실상 수렴한다.** 과거 3장(t−0.5/−1.0/−1.5s)이면 7-frame 의 거의
전부를 얻는다. depth 3 의 realized 0.2777 은 sparse KEEP 게이트(≤0.30) 안이다.

## 반드시 붙는 단서

depth 0 의 7.28 은 분포 이탈이 섞인 값이다. dense VAD 는 prev_bev 가 있는 조건에서만
학습됐으므로 prev_bev=None 은 학습 중 한 번도 없던 입력이다. 따라서 7.28 을
"t0 의 정보량 상한"으로 읽으면 과대평가다. 다만 곡선의 **모양**(0→3 에서 대부분,
3→6 에서 거의 없음)은 그와 무관하게 유효하다.

## 우리 sparse 모델의 재평가

current-only sparse 의 최고 B slO@12 는 0.5190(c2_kd). 위 표에서 dense 는
depth 1 에서 1.1142, depth 2 에서 0.4485 다.

**즉 우리 단일프레임 sparse 모델은 dense 에게 과거 1장을 준 것보다 낫고 2장 준 것에
근접한다.** ⑤-C~⑤-C3 를 "일반화 실패"로만 읽었으나, 한 장이 가진 정보량 대비로는
이미 상한 근처였다. 표현력 부족이 아니라 **입력 정보 부족**이다.

## 세 실패가 같은 원인이었다

| 시도 | 요구한 정보 | t0 한 장에 있는가 |
|---|---|---|
| 랭킹 loss (실제 주행 경로 top-1) | 자차 속도·정지·운전자 의도 | 없음 |
| logit KD (7-frame temporal teacher 모방) | teacher 의 temporal state | 없음 |
| occupancy 보조 (t=0.5~3.0s 미래 점유) | 타차 속도 | 없음 |

occ-only 진단(랭킹 loss 완전히 끔)에서 BCE 가 0.82~0.91 에서 멈춘 것도
(무작위 ~1.09) 같은 이유다. 미래 점유는 에이전트 속도를 요구하는데 한 장에서
관측되지 않는다. 세 실험이 독립적으로 같은 벽에 부딪혔다.

## ⑤-C3 사슬 검증 결과 (offset 샘플링)

설계 §6.3 의 4-offset 표본을 구현해 검증했고 가설은 1번 고리에서 끊어졌다.

| 검증 | 결과 |
|---|---|
| offset 이 점유 학습을 가능하게 하나 | 아니오. occ BCE 0.910(offset) vs 0.823(없음) |
| offset 이 B 를 개선하나 | 아니오. o_base 0.5667 vs 대조군 n_ctrl 0.5361 |
| stride 4 추가 | o_p1_occ1 0.5219 (−0.014, 편차 0.034 내) |
| probe A/B 격차 | 불변 (A≈0.120, B 0.95~1.05) |

aliasing 은 실재했고(69%→85%) §6.3 미구현은 실제 누락이었으나, **아무것도 바꾸지
못했다.** 해상도는 원인이 아니다.

## 다음: ⑤-D temporal sparse model

t0(768×432) + t−0.5/−1.0/−1.5(저해상도 가능)를 **shared backbone 에 한 번의 batch
forward** 로 처리하고(7-frame 순차 아님), 후보 경로별 temporal feature 를 정렬해
차분/attention 후 shortlist 12 를 낸다.

지연시간: ⑤-B 실측에서 backbone+FPN 이 13.003ms 중 9.463ms. 4프레임 공유 backbone 이면
대략 9.463×4 + 3.5 ≈ 41ms, 과거 프레임 저해상도면 더 낮다. **100ms 게이트에 여유.**

compliance: 상대 pose 는 과거 feature 를 현재 후보 좌표에 정렬하는 고정 행렬 곱에만
