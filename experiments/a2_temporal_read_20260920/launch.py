"""Launch one authorized matched-budget temporal-read run and preserve receipts."""
...
def main():
    ...
    if used!=0:
        raise RuntimeError(f'GPU{args.gpu} is in use: {used} MiB')
    cmd=[PYTHON,'-u',str(Path(__file__).with_name('train_temporal.py')),
    ...
    state=dict(phase=phase,physical_gpu=args.gpu,run_dir=str(run),log=str(log),command=cmd,
        ...
        status='launching',updates=2 if args.smoke else 20554)
