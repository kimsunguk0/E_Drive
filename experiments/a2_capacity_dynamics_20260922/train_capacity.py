"""Matched DEV capacity, full decoder split and agent-future supervision."""
...
for p in (ROOT,ROOT/'scripts',ROOT/'experiments/a2_progress_h4_20260920',ROOT/'experiments/a2_native_detail_20260921',HERE):sys.path.insert(0,str(p))
...
from capacity_model import CapacityModel,ARMS,arm_config,load_parent,NEW_BLOCK_PREFIX
...
PARENT_RUN=ROOT/'work_dirs/a2_native_detail_20260921/M-NATIVE-s1'
PARENT=PARENT_RUN/'ckpt_step10277.pth'
PARENT_SHA='8bc43364c05c899a563438f34538845ed0c33a969ab7e0cb6786e407ed9cbadf'
...
def configure(arm,steps=UPDATES):
    ...
    class Factory:
        ...
        def __new__(cls,config,*,arm):return CapacityModel(config,execution_config=arm_config(arm))
    with nominal.configure('A2-BASE-NOM',MICRO):
        legacy.SharedDynamicsMotionDriveV2=Factory
        legacy.CausalStatusDataset=lambda base:NativeDataset(H4StatusDataset(agent_data.AgentDataset(base) if arm=='C-AGENT' else base),'M-NATIVE')
...
def declaration(arm,gpu,run,steps):
    ...
    if pm['status']!='completed' or pm['step']!=10277 or pm['nonfinite_count']:raise ValueError('Bad parent')
    ...
    for k in ('geometry','initial_load','extension_policy','full_transfer_policy','expected_shared_base_initial_sha256','optimizer_native_group'):
        d.pop(k,None)
    spec=arm_config(arm);ns=spec['parent_native_execution_config']
    d.update(name='a2_capacity_dynamics_20260922',arm=arm,physical_gpu=gpu,run_dir=str(run),smoke=steps!=UPDATES,
        native_execution_config=ns,next_execution_config=ns['parent_next_execution_config'],
        execution_config=ns['parent_next_execution_config']['base_execution_config'],capacity_execution_config=spec,
        parent_updates=10277,stage_updates=steps,training_aux_in_state=arm=='C-AGENT',
        ...
        initializer={'path':str(PARENT),'checkpoint_sha256':PARENT_SHA,'model_state_sha256':tensor_state_sha256(cp['model']),
            'source':'completed DEV M-NATIVE terminal; weights and buffers only'},
        expected_optimizer_groups=[{'name':'backbone','base_lr':1e-6},{'name':'head','base_lr':1e-5}],
        question='One changed axis: grown image trunk, independent progress/heading queries/decoders, or agent future visual supervision',
        control={'arm':'C-CTRL','parent_PREFIX':.14459766188205064,'primary':'terminal arm minus CTRL AND frozen parent'},
        comparison_contract={'same_mapped_parent_tensors':True,'same_rows_baseline_pixels_jitter_flip':True,
            'same_updates_existing_optimizer_schedule':True,'same_native_detail_pixels':True,'FULL_leakage':False},
        automatic_followups=False,automatic_followup_training=False,automatic_FULL=False,
        interpretation='Existing image inputs and objectives fixed; each treatment compared with same-parent same-budget control.',
        source_commit=subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),
        source_tree=subprocess.check_output(['git','rev-parse','HEAD^{tree}'],cwd=ROOT,text=True).strip())
    d['recipe'].pop('new_detail_lr',None)
    d['recipe'].update(updates=steps,warmup=100,head_lr=1e-5,backbone_lr=1e-6,new_residual_block_lr=1e-5,new_agent_head_lr=5e-5,
    ...
    d['agent_auxiliary']={'enabled':arm=='C-AGENT','lambda':AGENT_LAMBDA,'ramp_updates':500,'target_policy':agent_data.POLICY,
        'cache_manifest_sha256':sha(agent_data.CACHE/'manifest.json') if arm=='C-AGENT' else None,'deployment_head':False}
...
def runtime(arm,d,run,protocol):
    ...
    probe=NativeDataset(H4StatusDataset(mr.MotionCanvasDataset(raw_probe,'native')),'M-NATIVE')
    ...
    with legacy.patched_runtime(arm,1,d,run,False):
        ...
        refs=[];row_sha=hashlib.sha256();base_sha=hashlib.sha256();high_sha=hashlib.sha256();calls=[0];step=[0];groups=[];grad=[];agent_norm=[None]
        def initialize(model,common,experiment=None):
            ...
            d['initial_load']={'parent_tensors_equal':True,'new_keys':extras,'optimizer_moments_restored':False,
                'parent_step':10277,'stage_step':0,'initial_state_sha256':d['expected_initial_model_state_sha256']}
        ...
        def enrich(m):
            m.update(execution_config=d['execution_config'],next_execution_config=d['next_execution_config'],
                native_execution_config=d['native_execution_config'],capacity_execution_config=arm_config(arm),
                training_aux_in_state=arm=='C-AGENT',parent_updates=10277,stage_updates=d['stage_updates'],
                ...
                stream_audit={'microcalls':calls[0],'rows_sha256':row_sha.hexdigest(),'baseline_augmentation_sha256':base_sha.hexdigest(),
                    'detail_pixels_sha256':high_sha.hexdigest(),'detail_pixels_expected_to_differ':False})
        ...
        class DetailAdamW(old_adam):
            def __init__(self,params,*a,**kw):
                ...
                names=['backbone','head']
                for prefix,lr,name in ((NEW_BLOCK_PREFIX,1e-5,'new_identity_blocks'),(('agent_future_head.',),5e-5,'agent_head')):
                    ps=[p for n,p in refs[0].named_parameters() if n.startswith(prefix)]
                    if ps:
                        ids={id(p) for p in ps}
                        for g in gs:g['params']=[p for p in g['params'] if id(p) not in ids]
                        gs.append({'params':ps,'lr':lr,'base_lr':lr});names.append(name)
                ...
                groups.extend({'name':n,'base_lr':g['base_lr'],'parameters':sum(p.numel() for p in g['params'])}
                              for n,g in zip(names,self.param_groups))
        ...
        def clip(ps,*a,**kw):
            ...
            if s<=5 or s in (100,500,3426,6852,10277):
                watch=('backbone_fpn.layer3.6.bn3.weight','backbone_fpn.layer3.6.conv1.weight','backbone_fpn.layer3.22.bn3.weight',
                    'planner.length_queries','planner.heading_queries','planner.length_decoder.layers.0.self_attn.in_proj_weight',
                    'planner.heading_decoder.layers.0.self_attn.in_proj_weight','agent_future_head.0.weight','agent_future_head.2.weight',
        ...
        old_norm=loss_api.build_loss_normalizers
        def normalizers(raw):
            if arm=='C-AGENT':agent_norm[0]=agent_data.normalizers(raw)
            return old_norm(raw)
        selected=wrap_compute_loss(common_loss,.25)
        def loss(outputs,batch,weights,**kw):
            total,parts=selected(outputs,batch,weights,**kw)
            assert 'plan_interval_length' in parts and 'plan_interval_vector' not in parts
            if arm=='C-AGENT':
                assert agent_norm[0] is not None
                value=agent_data.agent_loss(outputs['agent_future_field'],batch,agent_norm[0])
                coeff=AGENT_LAMBDA*min(1.,(step[0]+1)/500.)
                total=total+coeff*value;parts.update(agent_future=value,agent_future_weighted=coeff*value,total=total)
            return total,parts
        trainer.compute_loss=loss;loss_api.build_loss_normalizers=normalizers
        flip_api.flip_item=agent_data.wrap_flip(wrap_flip(old_flip,'M-NATIVE'));torch.optim.AdamW=DetailAdamW;torch.nn.utils.clip_grad_norm_=clip
        try:yield
        finally:
            flip_api.flip_item=old_flip;torch.optim.AdamW=old_adam;torch.nn.utils.clip_grad_norm_=old_clip
            loss_api.build_loss_normalizers=old_norm
