...
for rel in ('','scripts','experiments/md_r0_reset_20260914','experiments/md_shared_dynamics_20260917',
            ...
            'experiments/a2_progress_fourarm_20260921'):
...
def load_model(checkpoint,require_full=False,device=None):
    ...
    model,payload=load_export(checkpoint,device or torch.device('cuda:0'))
    protocol=payload['manifest']['experimental_protocol']
    if require_full:
        assert protocol['name']=='a2_progress_fourarm_full_20260921'
        assert protocol['full_fit']['enabled'] and payload['step']==4156 and not protocol['smoke']
    assert protocol['nominal_input']['producer_sha256']==sha(h4_status.__file__)
    assert model.progress_head is None
...
def main():
    ...
    result={'checkpoint_sha256':sha(args.checkpoint),'arm':payload['manifest']['experimental_protocol']['arm'],
        'execution_config':model.execution_config,'step':payload['step'],'device':name,'torch':torch.__version__,'cuda':torch.version.cuda,
        ...
        'imported_source_files':{module.__name__:str(Path(module.__file__).resolve()) for module in
            (sys.modules['arm_model'],sys.modules['fine_motion'],sys.modules['progress_model'],sys.modules['temporal_model'],sys.modules['motion_model'],
