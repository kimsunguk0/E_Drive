...
def planner_modes(model, x):
    ...
    try:
        # grad on: only params after the can_bus injection are trainable, so
        # autograd records just refine -> planner (frozen trunk carries no graph)
...
def main():
    ...
    for n, p in model.named_parameters():
        p.requires_grad_(n.startswith(('planner.', 'canbus_mlp.', 'scene_encoder.refine.')))
    new = [p for n, p in model.named_parameters() if n in NEW_KEYS]
    old = [p for n, p in model.named_parameters() if n.startswith(('planner.', 'scene_encoder.refine.')) and n not in NEW_KEYS]
