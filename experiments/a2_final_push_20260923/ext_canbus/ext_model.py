...
import os
K_LON = int(os.environ.get('EXT_K', '6'))
K_LAT = int(os.environ.get('EXT_LAT', '1'))
K = K_LON * K_LAT
LAT_SPREAD = float(os.environ.get('EXT_LAT_SPREAD', '0.03'))   # rad at 5.0 s
EXT = 4                       # 3.5, 4.0, 4.5, 5.0 s
SPREAD = float(os.environ.get('EXT_SPREAD', '0.03'))                # initial per-mode progress spread at 5.0 s (raw length units)
...
class ModeExtPlanner(ProgressHeadingPlanner):
    def __init__(self, config):
        ...
        lon = torch.linspace(-SPREAD, SPREAD, K_LON).repeat_interleave(K_LAT)
        lat = (torch.linspace(-LAT_SPREAD, LAT_SPREAD, K_LAT) if K_LAT > 1 else torch.zeros(1)).repeat(K_LON)
        self.mode_length_bias = nn.Parameter(lon[:, None] * ramp)
        self.mode_heading_bias = nn.Parameter(lat[:, None] * ramp)
    ...
    def forward(self, scene_features, motion_features, predicted_state, predicted_history,
        ...
        with torch.autocast(device_type=scene_features.device.type, enabled=False):
            ...
            hbias = self.mode_heading_bias.float()[None].expand(b, -1, -1).reshape(b * K, 6 + EXT)
            raw = torch.stack([raw[..., 0] + bias, raw[..., 1] + hbias], -1)
...
class ExtModel(H4ProgressModel):
    def __init__(self, config, *, arm=PROGRESS):
        ...
        # VAD-style can_bus: the provided causal status is ADDED to the shared
        # scene raster right before the common refinement (the same place the
        # global image context is added), like bev_queries + can_bus_mlp(can_bus)
        # in the official VAD baseline. Occupancy, lane and planner read the result;
        # the motion encoder never sees status. Zero-initialised: starts == parent.
        c = config.channels
        with torch.random.fork_rng(devices=devices):
            torch.manual_seed(2026092302)
            self.canbus_mlp = nn.Sequential(nn.Linear(5, 64), nn.GELU(), nn.Linear(64, c))
        nn.init.zeros_(self.canbus_mlp[-1].weight); nn.init.zeros_(self.canbus_mlp[-1].bias)
        self.register_buffer('canbus_scale', torch.tensor([10.0, 5.0, 3.0, 3.0, 0.5]), persistent=False)
        def add_canbus(_module, args):
            status = self._provided_status_context
            if status is None:
                raise RuntimeError('can_bus status context missing')
            (x,) = args
            with torch.autocast(device_type=x.device.type, enabled=False):
                delta = self.canbus_mlp(status.float() / self.canbus_scale)
            return (x + delta[:, :, None, None].to(x.dtype),)
        self.scene_encoder.refine.register_forward_pre_hook(add_canbus)
...
NEW_KEYS = ('planner.mode_length_bias', 'planner.mode_heading_bias', 'planner.mode_embed', 'planner.ext_queries',
            'planner.ext_delta.weight', 'planner.ext_delta.bias',
            'canbus_mlp.0.weight', 'canbus_mlp.0.bias', 'canbus_mlp.2.weight', 'canbus_mlp.2.bias')
