...
def main():
    ...
    torch.manual_seed(20260923); np.random.seed(20260923)
    ...
    loader = DataLoader(sub, batch_size=16, shuffle=True, num_workers=10, pin_memory=True, drop_last=True,
                        generator=torch.Generator().manual_seed(20260923), persistent_workers=not args.smoke)
    ...
    emit('plan', arm=args.arm, parent=PARENTS[args.arm], K=K, ext=EXT, train_rows=len(sub), steps=total,
