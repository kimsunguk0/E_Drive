...
class ModeExtPlanner(ProgressHeadingPlanner):
    def __init__(self, config):
        ...
        # Factorised candidates: every mode is (longitudinal i, lateral j) and
        # shares its i-part with all lateral siblings and its j-part with all
        # longitudinal siblings, so many candidates do not starve for data.
        lon_axis = torch.linspace(-SPREAD, SPREAD, K_LON)
        lat_axis = torch.linspace(-LAT_SPREAD, LAT_SPREAD, K_LAT) if K_LAT > 1 else torch.zeros(1)
        self.lon_length_bias = nn.Parameter(lon_axis[:, None] * ramp)
        self.lat_heading_bias = nn.Parameter(lat_axis[:, None] * ramp)
        self.lon_embed = nn.Parameter(torch.zeros(K_LON, 1, c))
        self.lat_embed = nn.Parameter(torch.zeros(K_LAT, 1, c))
    ...
    def forward(self, scene_features, motion_features, predicted_state, predicted_history,
        ...
        with torch.autocast(device_type=scene_features.device.type, enabled=False):
            ...
            mode_embed = (self.lon_embed.float().repeat_interleave(K_LAT, 0)
                          + self.lat_embed.float().repeat(K_LON, 1, 1))                 # K,1,c
            q6 = (self.waypoint_queries.float()[None] + mode_embed)                    # K,6,c
            qe = (self.ext_queries.float()[None] + mode_embed)                         # K,4,c
            ...
            bias = self.lon_length_bias.float().repeat_interleave(K_LAT, 0)[None].expand(b, -1, -1).reshape(b * K, 6 + EXT)
            hbias = self.lat_heading_bias.float().repeat(K_LON, 1)[None].expand(b, -1, -1).reshape(b * K, 6 + EXT)
...
NEW_KEYS = ('planner.lon_length_bias', 'planner.lat_heading_bias', 'planner.lon_embed', 'planner.lat_embed', 'planner.ext_queries',
