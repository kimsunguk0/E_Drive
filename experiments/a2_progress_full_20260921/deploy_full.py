...
import numpy as np
import torch
from infer_full import ROOT,load_model,prepare_clip,predict,configure,sha,mr
...
def cost(model,payload,prepared,folder,checkpoint):
    ...
    assert total==730044861120 and total<7053e9
    timing=[]
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--preflight-checkpoint');ap.add_argument('--preflight-only',action='store_true');args=ap.parse_args()
    assert os.environ.get('CUDA_VISIBLE_DEVICES') in ('0','1','2','3')
    assert bool(args.preflight_checkpoint)==args.preflight_only
    folder=REPORT/'deploy_preflight' if args.preflight_only else REPORT
    folder.mkdir(parents=True,exist_ok=True)
    assert not (folder/'raw_parity.json').exists(),'Do not overwrite completed parity'
    checkpoint=Path(args.preflight_checkpoint) if args.preflight_only else RUN/'ckpt_step24931.pth'
    ...
    if args.preflight_only:
        write(folder,'summary.json',{'status':'passed','weights':'selected DEV only for deployment preparation',
    ...
    subprocess.run([sys.executable,str(ROOT/'experiments/md_r0_reset_20260914/package_submission.py'),
        ...
        '--out-dir',str(PACKAGE),'--label','A2-H4-PROGRESS-FULL-s1'],check=True)
