...
SCENE_SOURCE_SHA = 'be8d9eef8e1e9f087852c1a81d0f5f4437e618a146d02d5037de60181b360ae5'
...
def counts_and_mode(manifest):
    ...
    require(schema in ('c_scene_selector_frozen_v1','c_scene_continuation_v1'),'Unknown refinement schema')
    if schema=='c_scene_selector_frozen_v1':
...
def inspect_scene_checkpoint(path, ev):
    ...
    require(m['source_sha256'].get('c_scene_selector.py')==SCENE_SOURCE_SHA,'Unsupported scene head implementation')
...
class FullSceneModel(nn.Module):
    def __init__(self, original_c, scene_head, scene_module):
        super().__init__()
...
def load_verified_models(scene,plan,runtime,ev):
    ...
    """
    mod=module_from(scene['source']/'c_scene_selector.py','_scene_head_recorded')
    ...
    model=FullSceneModel(original,head,mod).eval()
...
class CacheReference:
    def __init__(self,path,plan,scene,ev):
        ...
        m=self.manifest
        require(scene['manifest']['schema']=='c_scene_selector_frozen_v1','A frozen base cache cannot audit a continued model_c')
    ...
    def compare_batch(self, batch, output, goal, head, bank, start):
        ...
        repeated=head(offline,goal_xy=refs['goal_xy'])
...
def audit_goal_routes(model,original,inputs,ev):
    ...
    return dict(goal_only_after_complete_candidates=True,candidate_token_goal_invariant=True,
        original_route_counts=counts,new_head_signature='output, goal_xy; no raw or predicted status input')
