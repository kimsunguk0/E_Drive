...
PARENT=ROOT/'work_dirs/a2_progress_fourarm_20260921/P-CTRL-s1/final_eval.json'
ARMS=('P-CTRL-NEXT','P-SPLITREAD','P-LANE-GEOM');STEPS=(0,3426,6852)
...
def result(step):
    ...
    arrays={};out={'step':step,'primary':step==6852,'parent':parent_stats,'arms':{}}
    for arm in ARMS:
        ...
        out['arms'][arm]=a
    for arm in ARMS:
        a=out['arms'][arm];a['delta_ctrl']=compare(arrays[arm]-arrays['P-CTRL-NEXT'],session)
...
def summarize(rows,completed):
    ...
    terminal=next((r for r in rows if r['step']==6852),None)
    out={'status':'completed' if completed else 'running','terminal_primary_step':6852,
    ...
    if completed:
        assert st['last_compared_step']==6852 and st['steps_compared']==6852
        ...
        decision={'status':'DEV_complete','lowest_terminal_arm':best,'candidate_for_FULL':best if improve else None,
            ...
            'FULL_transfer_updates':8311 if improve else None,'FULL_started':False,'uploaded':False,'extension_requires_review':True,
    ...
    lines=['# CTRL 이후 SplitRead / LaneGeometry 동일 조건 continuation','',
        '완료: terminal6,852이 주 비교다.' if completed else '실행 중. 예정 중간 결과를 terminal로 해석하지 않는다.',
...
def main():
    ...
    while True:
        ...
        _,m=stream()
        done=len(m)==3 and all(v.get('status')=='completed' and v.get('step')==6852 and
            (RUNS/f'{a}-s1/experiment.json').exists() for a,v in m.items())
        if done:assert len(seen)==3 and all(v['nonfinite_count']==0 for v in m.values())
        ...
        if time.monotonic()-start>12*3600:raise TimeoutError('Collection deadline; jobs untouched')
        time.sleep(30)
