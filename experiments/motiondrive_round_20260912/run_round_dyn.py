...
NAME = "motiondrive_round_20260912"
...
# Every arm continues the same A2-off + flip parent. The provided causal status
# reaches nothing; the planner keeps its own image-inferred state and history,
...
#   dyn0  temporal scene adapter and future head, future loss coefficient 0
#   dyn1  the same graph and initialisation, future loss coefficient 0.2
ARMS = ("dyn0", "dyn1")
ARM_SEEDS = {arm: 0 for arm in ARMS}
FLIP_P = {arm: 0.5 for arm in ARMS}
PLANNER_LAYERS = {arm: 2 for arm in ARMS}
MODES = {}
FUTURE_WEIGHT = {"dyn0": 0.0, "dyn1": 0.2}
FUTURE_TARGETS = ROOT / "cache/motiondrive_round_20260912/future_targets"
...
# The pinned A2-off + flip terminal every arm in this round continues.
PARENT = {
    "checkpoint_sha256": "2e2fca25dff41cca62e2b24dfaef678b523585b5c2b880be37185342bc65fd44",
    "model_state_sha256": "af609201533d41e5462142b4dbff7d3c19f7caa17ad2f0c201b9f3d09b4c46fc",
    "sidecar_sha256": "4750d3bde5759ee2a0fc209c7e5614b26c215d61701773720101e3ef90a1a1f9",
    "source_manifest_sha256": "db1cf2423cd1c1afa800a31b534f16a32e9f7eaa3411047479fb3beb3266c499",
    "step": 6850,
    "protocol_name": "motiondrive_v2_q10_screen",
    "protocol_arm": "q10_flip50_s0",
}
SOURCE_FILES = set(a2.SOURCE_FILES) | {
    "scripts/motiondrive_v2_flip_augment.py",
    "experiments/motiondrive_round_20260912/run_round_dyn.py",
    "experiments/motiondrive_round_20260912/temporal_scene_adapter.py",
    }
...
def validate_parent(args):
    ...
    require(sha256(checkpoint) == PARENT["checkpoint_sha256"]
            ...
            "this round requires the exact pinned A2-off + flip parent and sidecar")
    ...
    # Inverted on purpose: this round's parent is the A2-off terminal, so
    # neither the A2 query conditioning nor the A1 FiLM may be present.
    require(not any(name.startswith(("shared_status_query", "shared_status_fusion"))
                    for name in payload["model"]),
            "the pinned parent must be A2-off and A1-free")
    return payload, {
...
_LIVE = {"planner": None}
GRID = (64, 48)
# Fixed once from the train split, clipped to [1, 20] as the spec requires.
POS_WEIGHT = (8.378682025870670, 8.317418739973835)


def expand_planner_depth(model, missing, layers):
    """Give the extra decoder layers an exact identity at initialisation.

    Only the residual output projections are zeroed - both attention out_proj
    tensors and the FFN's second linear - so each added layer returns its input
    unchanged while its internal Q/K/V, first linear and LayerNorms keep normal
    initialisation and can receive gradient once the outputs move.
    """
    if layers == 2:
        require(not missing, "the two-layer arm must load every parent tensor")
        return []
    decoder = model.planner.decoder
    require(len(decoder.layers) == layers, "decoder depth was not expanded")
    added = []
    for index in range(2, layers):
        layer = decoder.layers[index]
        for module in (layer.self_attn.out_proj, layer.multihead_attn.out_proj, layer.linear2):
            nn.init.zeros_(module.weight)
            if module.bias is not None:
                nn.init.zeros_(module.bias)
        added.append(index)
    expected = {f"planner.decoder.layers.{i}.{suffix}" for i in added
                for suffix in (n for n, _ in decoder.layers[added[0]].named_parameters())}
    unexpected = set(missing) - expected
    require(not unexpected, f"unexpected missing tensors outside the added layers: {sorted(unexpected)[:4]}")
    identities = {id(p) for p in decoder.layers[added[0]].parameters()}
    for index in added[1:]:
        require(not (identities & {id(p) for p in decoder.layers[index].parameters()}),
                "added decoder layers must not share Parameter objects")
    require(decoder.layers[0].norm_first and decoder.norm is None,
            "expected pre-norm layers and no trailing decoder norm")
    return added


class FutureTargetDataset(torch.utils.data.Dataset):
    """Attach the offline future footprint target, matching the flip decision.

    The flip wrapper is pinned source and cannot be edited, so its per-item
    decision is recomputed here from the same (seed, epoch, index) recipe and
    applied to the future arrays, keeping them in step with the images.
    """

    def __init__(self, base, split, flip_p, flip_seed):
        self.base, self.flip_p, self.flip_seed, self.epoch = base, flip_p, flip_seed, 0
        bundle = np.load(FUTURE_TARGETS / f"{split}.npz", allow_pickle=False)
        self.lookup = {int(r): i for i, r in enumerate(bundle["row"])}
        self.target = bundle["future_target"]
        self.valid = bundle["future_valid"]
        self.present = bundle["future_present"]
        for attribute in ("rows", "arr", "scene_names"):
    ...
    def __getattr__(self, name):
        if name in {"base", "flip_p", "flip_seed", "epoch", "lookup",
                    "target", "valid", "present"}:
            raise AttributeError(name)
        return getattr(self.base, name)

    def _flipped(self, index):
        if self.flip_p <= 0:
            return False
        state = np.random.RandomState((self.flip_seed * 1000003 + self.epoch * 7919
                                       + index) % (2 ** 31 - 1))
        return bool(state.rand() < self.flip_p)

    def __getitem__(self, index):
        item = self.base[index]
        row = int(item["row"])
        position = self.lookup.get(row)
        if position is None:
            target = np.zeros((2, *GRID), np.float32)
            valid = np.zeros((2, *GRID), bool)
        else:
            target = self.target[position].astype(np.float32)
            valid = self.valid[position].copy()
            if self._flipped(index):
                target = target[..., ::-1].copy()
                valid = valid[..., ::-1].copy()
        item["future_target"] = torch.from_numpy(target)
        item["future_valid"] = torch.from_numpy(valid)
        return item


def install_dynamic(model, config):
    """Swap in the dynamic-scene graph, keeping every parent tensor name."""
    sys.path.insert(0, str(Path(__file__).resolve().parent))
    from temporal_scene_adapter import DynamicSceneMotionDrive
    replacement = DynamicSceneMotionDrive(config)
...
def prepare_model(payload, seed):
    ...
    require(config.planner_layers == 2,
            "this round pins a two-layer parent; verify before comparing depth")
    ...
    model = install_dynamic(make_model(config), config)
    ...
    new_names = [name for name in incompatible.missing_keys
                 if not name.startswith(("scene_adapter", "future_head"))]
    ...
    require(not any(name.startswith(("shared_status_query", "shared_status_fusion"))
            ...
            "no arm may carry A2 query conditioning")
    dropped = 0
    return model, {
        ...
        "added_decoder_layers": PLANNER_LAYERS[_ARM["name"]] - 2,
    }
...
def status_dataset(base, split, overlay, augmentable=False):
    ...
    if _FLIP["on"] and augmentable:
        ...
        base = FlipAugmented(base, _FLIP["wf"], _FLIP["wh"],
                             p=_FLIP["p"], seed=_FLIP["seed"])
        return FutureTargetDataset(base, split, _FLIP["p"], _FLIP["seed"])
    return FutureTargetDataset(base, split, 0.0, _FLIP["seed"])
...
def build_experiment(args, source, data, parent, prepared):
    return {
        ...
        "future_loss_weight": FUTURE_WEIGHT[args.arm],
        "future_pos_weight": list(POS_WEIGHT),
...
def patched_runtime(arm, seed, overlay, holdout, expected_parent_sha,
    ...
    def model_factory(config=None):
        ...
        return install_dynamic(make_model(config), config)
    ...
    def dynamic_loss(outputs, batch, weights, *, normalizers=None, stop_class_weights=None):
        loss, parts = originals["compute_loss"](outputs, batch, weights,
                                                normalizers=normalizers,
                                                stop_class_weights=stop_class_weights)
        from temporal_scene_adapter import future_footprint_loss
        future = future_footprint_loss(outputs["future_logits"], batch["future_target"],
    ...
    trainer.compute_loss = dynamic_loss
