...
## 작업 C — EXP-LONG 결과 (2026-09-15, 완료): `TRAIN_BETTER_V0_NOT_BETTER`

1. **바꾼 것 한 가지**: 학습 예산과 cosine horizon(20,554 → 31,388). 나머지는 E1 그대로.
2. **유지한 것**: 같은 초기화, 같은 Tplus, 같은 V0 row, 구조·loss·LR·증강·seed 0, command OFF.
3. **실제 값**: LONG terminal **0.228941**, best 0.226355(23,982). short EXP는 0.225592.
   terminal 차이 **+0.003349**, CI [+0.00078, +0.00627], **4/11 세션만 개선** — 유의하게 나쁘다.
   그런데 LONG의 train probe는 **0.076272**로 short의 0.085851보다 좋다.
4. **확인된 것**: 확대 pool에 같은 epoch 수를 주는 것은 고정 계산량보다 낫지 않다.
   E1-T203(0.073538 / 0.267920)과 **같은 서명**이다 — 6회 노출까지 가면 두 pool 모두
   train 적합이 0.073–0.076으로 수렴하고 V0는 나빠진다. 확대의 이득은 노출이 아니라
   분포 다양성에서 왔고, 이 recipe에서 20,554 update는 이미 충분했다.
   **확인되지 않은 것**: 다른 LR·다른 정규화에서의 장기 학습. LONG의 seed 복제(단일 seed).
5. **다음 한 가지**: **short EXP를 후보로 유지**한다. LONG을 더 늘리지 않는다.
   §5에 따라 다음 작은 질문은 command OFF/ON 대조이며, 사실 확인은 끝났고 명세는 아직 없다.
   **H는 열지 않았다** — 설계를 지금 잠그고 H로 갈지, command 대조를 먼저 볼지가
   남은 갈림길이다. H는 1회용이므로 그 선택을 먼저 정해야 한다.
