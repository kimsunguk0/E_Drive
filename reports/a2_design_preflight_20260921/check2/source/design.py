...
def geometry_targets(polylines, centers, original_valid, policy=GEOMETRY_POLICY):
    ...
    for start in range(0, len(x), 128):
        ...
        for seg in groups:
            ...
            point_bad = (exact_segment & (np.linalg.norm(qs-ql[:, None],axis=-1)>policy['point_agreement_m'])).any(-1)
            ref_axis = segment_axes[ii]
            axis_bad = (exact_segment & ((segment_axes[None]*ref_axis[:, None]).sum(-1)<angle_cos)).any(-1)
        ...
        q_best = q[np.arange(len(xx)), index]
        axis_best = axes[np.arange(len(xx)), index]
        near = dd <= d[:, None]+policy['near_distance_m']
        exact = np.abs(dd-d[:, None]) <= policy['exact_distance_tie_m']
        point_ambiguous = (near & ((np.linalg.norm(q-q_best[:, None], axis=-1) > policy['point_agreement_m']) | internal_point_bad)).any(-1)
        axis_ambiguous = (near & (((axes*axis_best[:, None]).sum(-1) < angle_cos) | internal_axis_bad)).any(-1)
