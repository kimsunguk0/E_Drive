...
현재 상태(2026-09-07 22:31 KST): **P3 상태-query 두 팔1000step 정상 완주.
control0.44598 / state0.43794, 차이−0.00804로 사전−0.01 진전 기준 미달.**
11세션 paired95%구간[−0.01390,−0.00272], 단일seed·반복tune 조건이다.
P2 정상3팔 분리 진단은 완료했고, C1/T0의 저장 후 SIGSEGV(-11) 및 원4팔 gate 실패는
그대로 남는다. `MOTIONDRIVE_V2_P3_RESULTS_20260907.md`와
`MOTIONDRIVE_V2_P2_DIAGNOSTIC_RESULTS_20260907.md`를 최신 결과로 따른다.
최신 사용자 허용 범위는GPU0–5이며 실제 GPU4·5로 실행 후 현재 둘 다 유휴다.
GPU0–3의 기존 작업은 보존하고 GPU6·7과 타 작업 중지는 허용하지 않는다.
다음 우선순위는 수정 기하/nominal에서 공개 초기값부터의 baseline 재학습이며 아직 미발사다.
...
현재 실행 상태를 뜻하지 않는다. 아래 GPU0–3 제한도 후속 허용 이전의 기록이다.
