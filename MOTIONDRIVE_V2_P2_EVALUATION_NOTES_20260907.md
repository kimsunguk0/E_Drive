...
## 20:57 KST 추가: 종료 사고와 GPU4·5 사용 허용

위 20:38의 계획 이후 C1/T0가 LAST 저장 후 실제 SIGSEGV(-11)로 종료했다.
따라서 원 all-four clean-exit gate는 실패이며, 엄격 P2 분석기의 기준을 완화하지 않는다.
정상 종료한 세 팔만 별도의 사후 진단으로 평가한다. 상세 근거와 한계는
`MOTIONDRIVE_V2_P2_EXIT_INCIDENT_20260907.md`를 따른다.

사용자가 GPU4·5를 명시적으로 허용했다. 실제 두 장 모두 메모리0MiB·util0%·
compute PID 없음을 확인했다. 기존0–3의 다른 작업은 중지하지 않는다.
wrapper의 명시적 `--physical-gpu`는 평가 위치만 바꾸며 원 학습 GPU 검증과
데이터/배치/정밀도/조건 및 메모리·실제 종료 검사는 유지한다. GPU6·7은 금지한다.
세 팔의 평가 실행 commit은 동일하게 고정하고 결과에 실제 UUID/물리·논리 GPU를 남긴다.
이 부록 시점에 새 GPU 평가는 아직 실행하지 않았다.
